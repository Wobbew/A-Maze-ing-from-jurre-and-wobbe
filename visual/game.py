...
class player:
    ...
    def move(self, window):
        ...
        print(f"X={x} Y={y}")
        if self.is_valid((x, y)) and not self.get_wall(cells_around(self, 0, 0, 0), facing_to_bit(self.facing, "F")):
            self.X = x
            self.Y = y
...
def draw_left_wall(p, depth, window, max_depth = 4, wall = True):
    if depth == 3:
        ...
        if wall:
            ...
            if p.get_wall(cells_around(p, 1, 0, 1), facing_to_bit(p.facing, "F")):
        ...
        if wall:
            ...
            if p.get_wall(cells_around(p, 1, 0, 0), facing_to_bit(p.facing, "F")):
                mlx.mlx_put_image_to_window(ptr, window, get_image("images/FWall-1 1.xpm"), 0, 0)
                print("FWall-1 1.xpm")
            elif p.get_wall(cells_around(p, 1, 0, 1), facing_to_bit(p.facing, "L")):
                mlx.mlx_put_image_to_window(ptr, window, get_image("images/LWall-1 2.xpm"), 0, 0)
                print("FWall-1 2.xpm")
            else:                
                if p.get_wall(cells_around(p, 2, 0, 1), facing_to_bit(p.facing, "L")):    
                    mlx.mlx_put_image_to_window(ptr, window, get_image("images/LWall-2 2.xpm"), 0, 0)
                    print("LWall-2 2.xpm")
                if p.get_wall(cells_around(p, 2, 0, 1), facing_to_bit(p.facing, "F")):
...
def draw_right_wall(p, depth, window, max_depth=3, wall=True):
    if depth == 3:
        if wall:
            ...
            if p.get_wall(cells_around(p, 0, 1, 2), facing_to_bit(p.facing, "F")):
                mlx.mlx_put_image_to_window(ptr, window, get_image("images/FWall+1 3.xpm"), 0, 0)
    if depth == 2:
        if wall:
            ...
            if p.get_wall(cells_around(p, 0, 1, 1), facing_to_bit(p.facing, "F")):
                mlx.mlx_put_image_to_window(ptr, window, get_image("images/FWall+1 2.xpm"), 0, 0)
    if depth == 1:
        if wall:
            ...
            if p.get_wall(cells_around(p, 0, 1, 0), facing_to_bit(p.facing, "F")):
                mlx.mlx_put_image_to_window(ptr, window, get_image("images/FWall+1 1.xpm"), 0, 0)
            elif p.get_wall(cells_around(p, 0, 1, 1), facing_to_bit(p.facing, "R")):
                    mlx.mlx_put_image_to_window(ptr, window, get_image("images/RWall+1 2.xpm"), 0, 0)
            else:
                if p.get_wall(cells_around(p, 0, 2, 1), facing_to_bit(p.facing, "R")):    
                    mlx.mlx_put_image_to_window(ptr, window, get_image("images/RWall+2 2.xpm"), 0, 0)
                if p.get_wall(cells_around(p, 0, 2, 1), facing_to_bit(p.facing, "F")):
                    if p.get_wall(cells_around(p, 0, 0, 1), facing_to_bit(p.facing, "R")):
                        mlx.mlx_put_image_to_window(ptr, window, get_image("images/FWall+2 2.xpm"), 0, 0)
                    else:
                        mlx.mlx_put_image_to_window(ptr, window, get_image("images/FWall+2 2.xpm"), 0, 0)
...
def render_path(path, entry, maze):
    ...
    p = player(Y, X, "N", maze)
    window = mlx.mlx_new_window(ptr, 1920, 1080, "test")
    state = {"i": 0, "last_time": time.time()}
    def on_key(keynum, param):
        if keynum == KEY_ESCAPE:
            mlx.mlx_destroy_window(ptr, window)
            mlx.mlx_loop_exit(ptr)
    mlx.mlx_key_hook(window, on_key, None)
    mlx.mlx_loop_hook(ptr, on_loop, None)
    mlx.mlx_loop(ptr)
...
def draw_right_wall(p, depth, window, wall=True):
    if depth == 3:
        if wall:
            mlx.mlx_put_image_to_window(ptr, window, get_image("images/RWall3.xpm"), 0, 0)
        else:
