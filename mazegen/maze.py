...
class MazeGenerator:
    ...
    def verify(
        ...
        if perfect != 'False' and perfect != 'True':
    ...
    def inperfect(self) -> None:
        ...
        def remove_random(cell: Tuple[int, int], logo_cells: set) -> None:
            x, y = cell
        ...
        for y in range(self.height):
            for x in range(self.width):
                if is_dead_end(self.list_dict[y][x]['walls']):
                    dead_ends.append((x, y))

        random.shuffle(dead_ends)
        num_remove = max(1, len(dead_ends) // 2)

        for i in range(num_remove):
            remove_random(dead_ends[i], logo_cells)

        for special in (self.entry, self.exit):
            if is_dead_end(self.list_dict[special[1]][special[0]]['walls']):
                remove_random(special, logo_cells)
